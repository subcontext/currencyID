...
Dollar bill reconizer 

Based on HandGestureRecognizer by Greg Borenstein, October 2012
...
void setup() {
  ...
  for (int i = 0; i < trainingFilenames.length; i++) {
    ...
    if (gestureLabel.equals("1")) {
      labels[i] = 1;
    }

    if (gestureLabel.equals("5")) {
      labels[i] = 2;
    }

    if (gestureLabel.equals("10")) {
      labels[i] = 3;
    }

    if (gestureLabel.equals("no")) {
      labels[i] = 4;
    }
  ...
  model.saveModel("bills_model.txt");
